...
ALIAS_MANAGER::Gen_alias_id(WN *wn, POINTS_TO *pt)
{
  ...
  if ((id = pt->Id()) == 0) {
    ...
    Set_id(wn, id);
    if (_trace) {
      fprintf(TFile, "gen_alias_id<%" PRIuPTR "(map %d)>\n", id, WN_map_id(wn));
      pt->Print(TFile);
      fdump_tree(TFile, wn);
      fprintf(TFile,"aliased_with<%" PRIuPTR ",{",id);
    ...
    Set_id(wn, id);
    if (_trace) {
      fprintf(TFile, "set_alias_id<%" PRIuPTR "(map %d)>\n", id, WN_map_id(wn));
      pt->Print(TFile);
      fdump_tree(TFile, wn);
    }
  }
...
ALIAS_MANAGER::Note_invalid_ip_alias_class(const WN *wn)
{
  ...
  if (alias_class != PESSIMISTIC_AC_ID &&
    ...
    if (Get_Trace( TP_GLOBOPT, ALIAS_TRACE_FLAG)) {
      fprintf(TFile, "Alias manager: Noting IP alias class %" PRIuPTR " invalid\n",
	      alias_class);
    }
...
void Print_alias_info(char *buf, const ALIAS_MANAGER *am, const WN *wn)
{
  ...
  if (pt->Expr_kind() == EXPR_IS_ADDR && pt->Base_kind() == BASE_IS_FIXED) {
    if (pt->Ofst_kind() == OFST_IS_FIXED) 
      sprintf(buf, "id:%" PRIuPTR " %s+0x%" SCNx64 "",
	      alias_id, 
	      ST_class(pt->Base())==CLASS_VAR ? ST_name(pt->Base()) : "not_variable",
	      pt->Byte_Ofst());
    else
      sprintf(buf, "id:%" PRIuPTR " %s", alias_id, ST_name(pt->Base()));
  } else if (pt->F_param() && pt->Based_sym() != NULL) {
    sprintf(buf, "id:%" PRIuPTR " parm:%s", alias_id, ST_name(pt->Based_sym()));
  } else if (pt->Unique_pt() && pt->Based_sym() != NULL) {
    sprintf(buf, "id:%" PRIuPTR " uniq:%s", alias_id, ST_name(pt->Based_sym()));
  } else 
    sprintf(buf, "id:%" PRIuPTR , alias_id);
}
