				 ...
				 OPS* ops )
{
  ...
  // 14840, 14846: Generate correct, efficient code if result matches
  // true_tn or false_tn.  Also 091 and 1325.
  if ( result == false_tn ) {
  ...
  OPS *ops)
{
  ...
  if (non_sse2_fp ||
    ...
    // 14840: Generate correct and efficient code if dest_tn is
    // true_tn or false_tn
    if( dest_tn == false_tn ){
      TN* tmp = Build_TN_Like( false_tn );
					  ...
					  TYPE_ID select_type, OPS* ops )
{
  ...
  // 14840: Generate correct, efficient code if result matches
  // true_tn or false_tn
  if (result == false_tn) {
    variant = Negate_BR_Variant(variant);
    false_tn = true_tn;
  } else if (result != true_tn) {
    Expand_Copy( result, true_tn, select_type, ops );
  }
        ...
        OPCODE compare, OPS *ops)
{
  ...
  FmtAssert(MTYPE_is_integral(rtype) && 
  	    (MTYPE_is_float(desc) || desc == MTYPE_V16I4) &&
  ...
  if (MTYPE_is_float(desc)) {
    ...
    Build_OP( (OPCODE_desc(compare) == MTYPE_V16I8 ) ? TOP_cmppd : TOP_cmpps,
    ...
    case OPR_EQ: 
      Build_OP(TOP_cmpeq128v32, result, cmp_kid1, cmp_kid2, ops);
      break;
    case OPR_NE:
      Build_OP(TOP_cmpeq128v32, result, cmp_kid1, cmp_kid2, ops);
      rev_condition = TRUE;
      break;
    case OPR_GT:
      Build_OP(TOP_cmpgt128v32, result, cmp_kid1, cmp_kid2, ops);
      break;
    case OPR_LE: 
      Build_OP(TOP_cmpgt128v32, result, cmp_kid1, cmp_kid2, ops);
      rev_condition = TRUE;
      break;
    case OPR_LT:
      Build_OP(TOP_cmpgt128v32, result, cmp_kid2, cmp_kid1, ops);
      break;
    case OPR_GE:
      Build_OP(TOP_cmpgt128v32, result, cmp_kid2, cmp_kid1, ops);
      rev_condition = TRUE;
    ...
    if (rev_condition) {
      TCON then = Host_To_Targ (MTYPE_I4, 0xffffffff);
      TCON now  = Create_Simd_Const (MTYPE_V16I4, then);
      ST *sym = New_Const_Sym (Enter_tcon (now), Be_Type_Tbl(TCON_ty(now)));
      Allocate_Object(sym);
      TN *sym_tn = Gen_Symbol_TN(sym, 0, 0);
      TN *tmp = Build_TN_Like(result);
      Exp_Load(MTYPE_V16I4, MTYPE_V16I4, tmp, TN_var(sym_tn), TN_offset(sym_tn),
      	       ops, 0);
      Build_OP (TOP_xor128v32, result, tmp, result, ops);
    }
  }
}
        ...
        OPCODE compare, TN *cmp_kid1, TN *cmp_kid2, VARIANT variant, OPS *ops)
{
  ...
  // 14840: Code below must correctly handle result == true_tn or false_tn
  ...
  if( MTYPE_is_float(select_type) &&
  ...
  } else if( (MTYPE_is_float(select_type) || MTYPE_is_vector(select_type)) &&
      Is_Target_SSE2()            &&
    ...
    */
    if( MTYPE_is_integral(desc) && !MTYPE_is_vector(desc)){
	...
	FmtAssert(desc == MTYPE_V16I4,
		  ("Expand_Select_And_COndition: unexpected vector type"));
	switch( compare_opr ){
	case OPR_EQ: 
	  Build_OP(TOP_cmpeq128v32, tmp1, cmp_kid1, cmp_kid2, &new_ops);
	  break;
	case OPR_NE:
	  Build_OP(TOP_cmpeq128v32, tmp1, cmp_kid1, cmp_kid2, &new_ops);
	  rev_condition = TRUE;
	  break;
	case OPR_GT:
	  Build_OP(TOP_cmpgt128v32, tmp1, cmp_kid1, cmp_kid2, &new_ops);
	  break;
	case OPR_LE: 
	  Build_OP(TOP_cmpgt128v32, tmp1, cmp_kid1, cmp_kid2, &new_ops);
	  rev_condition = TRUE;
	  break;
	case OPR_LT:
	  Build_OP(TOP_cmpgt128v32, tmp1, cmp_kid2, cmp_kid1, &new_ops);
	  break;
	case OPR_GE:
	  Build_OP(TOP_cmpgt128v32, tmp1, cmp_kid2, cmp_kid1, &new_ops);
	  rev_condition = TRUE;
    ...
    if ( MTYPE_is_vector(select_type) && MTYPE_is_integral(select_type)) {
      /* 
	 tmp1 := icompare  <cmp_kid1>, <cmp_kid2> (generated already)
	 tmp2 := pxor <true_tn> <false_tn>
	 tmp3 := pand  <tmp1>, <tmp2>
	 <result> := pxor <tmp3>, <false_tn>
      */
      Build_OP(TOP_xor128v32, tmp2, true_tn, false_tn, &new_ops);
      if (! rev_condition) {
	Build_OP(TOP_and128v32, tmp3, tmp1, tmp2, &new_ops);
	Build_OP(TOP_xor128v32, result, tmp3, false_tn, &new_ops);
      }
      else {
	Build_OP(TOP_and128v32, tmp3, tmp1, tmp2, &new_ops);
	Build_OP(TOP_xor128v32, result, tmp3, true_tn, &new_ops);
      }
    }
    else {
      if (! rev_condition) {
	Build_OP( is_rsize_double ? TOP_andpd : TOP_andps, tmp2, tmp1, true_tn, &new_ops );
	Build_OP( is_rsize_double ? TOP_andnpd : TOP_andnps, tmp3, tmp1, false_tn, &new_ops );
      }
      else {
	Build_OP( is_rsize_double ? TOP_andnpd : TOP_andnps, tmp2, tmp1, true_tn, &new_ops );
	Build_OP( is_rsize_double ? TOP_andpd : TOP_andps, tmp3, tmp1, false_tn, &new_ops );
      }
    ...
    // 14840: Generate correct, efficient code if result matches
    // true_tn or false_tn.  Also 091 and 1325.
    OPERATOR cmp_opr = compare_opr;
    if ( result == true_tn && result != false_tn &&
	 ! ( Force_IEEE_Comparisons && MTYPE_is_float(select_type) ) ) {
      // Swap true_tn and false_tn and branch (cmov) condition
      TN *tmp = true_tn;
      true_tn = false_tn;
      false_tn = tmp;
      switch ( compare_opr ) {
      case OPR_EQ:  cmp_opr = OPR_NE;  break;
      case OPR_NE:  cmp_opr = OPR_EQ;  break;
      case OPR_LT:  cmp_opr = OPR_GE;  break;
      case OPR_LE:  cmp_opr = OPR_GT;  break;
      case OPR_GT:  cmp_opr = OPR_LE;  break;
      case OPR_GE:  cmp_opr = OPR_LT;  break;
      }
    }
    ...
    if( OP_NEED_PAIR( desc ) ){
      Expand_Split_Select( result, cmp_opr, cmp_opcode,
      ...
      // 14847, 330: If we have to strictly honor all IEEE comparisons,
      // we may need to alter the conditional move operation.

      BOOL extra_cmovp = FALSE;
      if ( Force_IEEE_Comparisons && TOP_is_flop(cmp_opcode) &&
	   cmov_top != TOP_cmova && cmov_top != TOP_cmovae ) {

	if ( TOP_is_x87( cmov_top ) ) {
	  DevWarn( "Exp_Select_And_Condition: %s is not supported yet.\n",
		   TOP_Name(cmov_top) );
	} else {
	  TN *tmp;
	  switch (cmov_top) {
	    // IEEE-754 rules:
	    // The comparisons EQ, GT, GE, LT, and LE, when either or
	    //   both operands is NaN returns FALSE.
	    // Comparisons ignore the sign of 0 (so +0 equals -0).
	    // The comparison NE, when either or both operands is NaN
	    //   returns TRUE.
	    // Comparisons of any non-NaN value against +/- INF return
	    //   the correct result.
	    // But, on a NaN, fucomi sets all flags: ZF, PF, CF

	    // Convert cmovb and cmovbe to cmova and cmovba by
	    // reversing the compare arguments;  WHY?
	  case TOP_cmovb:
	    cmov_top = TOP_cmova;
	    tmp = cmp_kid1;  cmp_kid1 = cmp_kid2;  cmp_kid2 = tmp;
	    break;
	  case TOP_cmovbe:
	    cmov_top = TOP_cmovae;
	    tmp = cmp_kid1;  cmp_kid1 = cmp_kid2;  cmp_kid2 = tmp;
	    ...
	    // For cmovne, append an extra cmovp;
	    // WHY NOT convert to cmove?
	  case TOP_cmovne:
	    extra_cmovp = TRUE;
	    break;

	    // Convert cmove to cmovne (by swapping true_tn and false_tn),
	    // then appending an extra cmovp.  WHY?
	  case TOP_cmove:
	    cmov_top = TOP_cmovne;
	    tmp = true_tn;  true_tn = false_tn;  false_tn = tmp;
	    extra_cmovp = TRUE;
	    break;

	  default:
	    FmtAssert( FALSE, ("Exp_Select_And_Condition: unsupported"
			       " opcode (%s)", TOP_Name(cmov_top)) );
	  }
	}
      }
	...
	true_tn_hi = Get_TN_Pair(true_tn);
      ...
      if ( extra_cmovp ) {
	Expand_Cmov( TOP_cmovp, result, true_tn, rflags, &new_ops,
		     result_hi, true_tn_hi );
      }
    }
  }
...
Expand_Shuffle (OPCODE opc, TN* result, TN* op1, VARIANT variant, OPS *ops)
{
  ...
  case OPC_V8I4V8I4SHUFFLE:
    Build_OP(TOP_pshufd, result, op1, Gen_Literal_TN(0xE1, 1), ops);
    break;
...
//****************************************************************************
  ...
  case INTRN_PSHUFHW:
   ...
   case INTRN_LDDQU:
    ...
    break;
   case INTRN_MOVSHDUP:
    Build_OP(TOP_fmovshdup, result, op0, op1, ops );
    break;
   case INTRN_MOVSLDUP:
  ...
  case INTRN_CRC32W:
	 ...
	 break;
  case INTRN_CRC32L:
	 Build_OP(TOP_crc32l,result, op0, op1, ops);
	 break;
  case INTRN_CRC32Q:
  ...
  case INTRN_PABSD:
    ...
    break;
  case INTRN_PMADDUBSW128:
    Build_OP(TOP_pmaddubsw128, result, op0, op1, ops );
    break;
  case INTRN_PMADDUBSW:
    Build_OP(TOP_pmaddubsw, result, op0, op1, ops );
    break;
  case INTRN_PMULHRSW128:
    Build_OP(TOP_pmulhrsw128, result, op0, op1, ops );
    break;
  case INTRN_PMULHRSW:
    Build_OP(TOP_pmulhrsw, result, op0, op1, ops );
    break;
	case INTRN_PALIGNR128:
		Expand_Intrinsic_Imm_Param(TOP_palignr128, result, op0, op1, op2, ops, 5);
		break;
	case INTRN_PALIGNR:
		Expand_Intrinsic_Imm_Param(TOP_palignr, result, op0, op1, op2, ops, 7);
		break;
...
#include "expand_avx.cxx"
  ...
  case INTRN_PAND128:
   ...
   case INTRN_PANDN128:
    ...
    break;
	case INTRN_POR128:
		Build_OP(TOP_por128, result, op0, op1, ops);
		break;
   case INTRN_PEXTRW64:
...
TN* Exp_Fetch_and_And(TN* addr, TN* opnd1, TYPE_ID mtype, OPS* ops,
                      LABEL_IDX *label, OPS *loop_ops);
TN* Exp_Fetch_and_Nand(TN* addr, TN* opnd1, TYPE_ID mtype, OPS* ops,
                      LABEL_IDX *label, OPS *loop_ops);
TN* Exp_Fetch_and_Or(TN* addr, TN* opnd1, TYPE_ID mtype, OPS* ops,
                     LABEL_IDX *label, OPS *loop_ops);
TN* Exp_Fetch_and_Xor(TN* addr, TN* opnd1, TYPE_ID mtype, OPS* ops,
                      LABEL_IDX *label, OPS *loop_ops);
TN* Exp_And_and_Fetch(TN* addr, TN* opnd1, TYPE_ID mtype, OPS* ops,
                      LABEL_IDX *label, OPS *loop_ops);
TN* Exp_Nand_and_Fetch(TN* addr, TN* opnd1, TYPE_ID mtype, OPS* ops,
                      LABEL_IDX *label, OPS *loop_ops);
TN* Exp_Or_and_Fetch(TN* addr, TN* opnd1, TYPE_ID mtype, OPS* ops,
                     LABEL_IDX *label, OPS *loop_ops);
TN* Exp_Xor_and_Fetch(TN* addr, TN* opnd1, TYPE_ID mtype, OPS* ops,
                      LABEL_IDX *label, OPS *loop_ops);
TN* Exp_Val_Compare_and_Swap(TN* addr, TN* opnd1, TN* opnd2, TYPE_ID mtype, OPS* ops);
TN* Exp_Bool_Compare_and_Swap(TN* addr, TN* opnd1, TN* opnd2, TYPE_ID mtype, OPS* ops);
TN* Exp_Lock_Test_and_Set(TN* addr, TN* opnd1, TYPE_ID mtype, OPS* ops);
void Exp_Lock_Release(TN* addr, TYPE_ID mtype, OPS* ops);
...
TN *
Exp_Intrinsic_Call (WN *intrncall, TN *op0, TN *op1, TN *op2,
                    OPS *ops, LABEL_IDX *label, OPS *loop_ops)
{
 ...
 //SSE4A instrinsics
  ...
  case INTRN_FETCH_AND_AND_I4:
  case INTRN_FETCH_AND_AND_I8:
    return Exp_Fetch_and_And(op0, op1, WN_rtype(intrncall),
                             ops, label, loop_ops);

  case INTRN_FETCH_AND_OR_I4:
  case INTRN_FETCH_AND_OR_I8:
    return Exp_Fetch_and_Or(op0, op1, WN_rtype(intrncall),
                            ops, label, loop_ops);

  case INTRN_FETCH_AND_XOR_I4:
  case INTRN_FETCH_AND_XOR_I8:
    return Exp_Fetch_and_Xor(op0, op1, WN_rtype(intrncall),
                             ops, label, loop_ops);
  
  case INTRN_FETCH_AND_NAND_I4:
  case INTRN_FETCH_AND_NAND_I8:
    return Exp_Fetch_and_Nand(op0, op1, WN_rtype(intrncall),
                              ops, label, loop_ops);
  ...
  case INTRN_VAL_COMPARE_AND_SWAP_I4:
  case INTRN_VAL_COMPARE_AND_SWAP_I8:
    return Exp_Val_Compare_and_Swap(op0, op1, op2,
  ...
  case INTRN_AND_AND_FETCH_I4:
  case INTRN_AND_AND_FETCH_I8:
    return Exp_And_and_Fetch(op0, op1, WN_rtype(intrncall),
                             ops, label, loop_ops);

  case INTRN_OR_AND_FETCH_I4:
  case INTRN_OR_AND_FETCH_I8:
    return Exp_Or_and_Fetch(op0, op1, WN_rtype(intrncall),
                            ops, label, loop_ops);

  case INTRN_XOR_AND_FETCH_I4:
  case INTRN_XOR_AND_FETCH_I8:
    return Exp_Xor_and_Fetch(op0, op1, WN_rtype(intrncall),
                             ops, label, loop_ops);
  
  case INTRN_NAND_AND_FETCH_I4:
  case INTRN_NAND_AND_FETCH_I8:
    return Exp_Nand_and_Fetch(op0, op1, WN_rtype(intrncall),
                              ops, label, loop_ops);
  ...
  case INTRN_LOCK_RELEASE_I4:
  case INTRN_LOCK_RELEASE_I8:
    mtype = (id == INTRN_LOCK_RELEASE_I4) ?  MTYPE_I4 : MTYPE_I8;
    Exp_Lock_Release(op0, mtype, ops);
    break;
...
TOP Compare_and_Swap_Top(TYPE_ID mtype) {
  switch( mtype ){
  case MTYPE_I1:
  case MTYPE_U1:
    return TOP_lock_cmpxchg8;

  case MTYPE_I2:
  case MTYPE_U2:
    return TOP_lock_cmpxchg16;

  case MTYPE_I4:
  case MTYPE_U4:
    return TOP_lock_cmpxchg32;

  case MTYPE_I8:
  case MTYPE_U8:
    return TOP_lock_cmpxchg64;

  default:
    FmtAssert( FALSE,
	       ("Compare_and_Swap_Top: support me now") );
  }
}
...
{
  TN* rflags = Rflags_TN();
  TOP top = Compare_and_Swap_Top(mtype);
...
TN* Exp_Bool_Compare_and_Swap(TN* addr, TN* opnd1, TN* opnd2, TYPE_ID mtype, OPS* ops)
{
  ...
  TN* result = Build_TN_Of_Mtype (Is_Target_64bit() ? MTYPE_I8: MTYPE_I4);
  TN* tmp = Build_TN_Like(opnd1);
...
// Expand XXX_AND_FETCH intrinsic using lock cmpxchg instruction
TN* Exp_XXX_and_Fetch(TOP intrn_top, TN* addr, TN* opnd1, TYPE_ID mtype,
                      OPS* ops, LABEL_IDX *label, OPS *loop_ops)
{
  TN* result_tn = Build_TN_Like(opnd1);
  TN* rflags = Rflags_TN();
  TOP top_cmpxchg = TOP_UNDEFINED;
  TOP top_ld = TOP_UNDEFINED;

  switch( mtype ){
  case MTYPE_I1:
  case MTYPE_U1:
    top_cmpxchg = TOP_lock_cmpxchg8;
    top_ld = TOP_ldu8_32;
    break;

  case MTYPE_I2:
  case MTYPE_U2:
    top_cmpxchg = TOP_lock_cmpxchg16;
    top_ld = TOP_ldu16_32;
    break;

  case MTYPE_I4:
  case MTYPE_U4:
    top_cmpxchg = TOP_lock_cmpxchg32;
    top_ld = TOP_ld32;
    break;

  case MTYPE_I8:
  case MTYPE_U8:
    top_cmpxchg = TOP_lock_cmpxchg64;
    top_ld = TOP_ld64;
    break;

  default:
    FmtAssert( FALSE,
	       ("Exp_XXX_and_Fetch: support me now") );
  }

  Build_OP(top_ld, result_tn, addr, Gen_Literal_TN(0, 4), loop_ops);

  TN* new_val = Build_TN_Of_Mtype(mtype);
  Build_OP(intrn_top, new_val, result_tn, opnd1, loop_ops);

  TN* tmp = Build_TN_Like(result_tn);
  Build_OP(top_cmpxchg, rflags, tmp, result_tn, new_val, addr, Gen_Literal_TN(0, 4), loop_ops);

  *label = Gen_Temp_Label();
  Build_OP(TOP_jne, rflags, Gen_Label_TN(*label, 0), loop_ops);
  return new_val;
}
                      ...
                      LABEL_IDX *label, OPS *loop_ops)
{
  ...
  default:
    FmtAssert( FALSE,
	       ("Exp_And_and_Fetch: support me now") );
  }

  return Exp_XXX_and_Fetch(top, addr, opnd1, mtype, ops,
                           label, loop_ops);
}

// FIXME: not sure if this will work for 8 and 16 bit types
TN* Exp_Nand_and_Fetch(TN* addr, TN* opnd1, TYPE_ID mtype, OPS* ops,
                       LABEL_IDX *label, OPS *loop_ops)
{
  TN* result_tn = Build_TN_Like(opnd1);
  TN* rflags = Rflags_TN();
  TOP top_and = TOP_UNDEFINED;
  TOP top_not = TOP_UNDEFINED;
  TOP top_cmpxchg = TOP_UNDEFINED;
  TOP top_ld = TOP_UNDEFINED;
  ...
  default:
    FmtAssert( FALSE,
	       ("Exp_Nand_and_Fetch: support me now") );
  }
                     ...
                     LABEL_IDX *label, OPS *loop_ops)
{
  ...
  default:
    FmtAssert( FALSE,
	       ("Exp_Or_and_Fetch: support me now") );
  }
                      ...
                      LABEL_IDX *label, OPS *loop_ops)
{
  ...
  default:
    FmtAssert( FALSE,
	       ("Exp_Xor_and_Fetch: support me now") );
  }
...
// Expand FETCH_AND_XXX intrinsic using lock cmpxchg instruction
TN* Exp_Fetch_and_XXX(TOP intrn_top, TN* addr, TN* opnd1, TYPE_ID mtype,
                      OPS* ops, LABEL_IDX *label, OPS *loop_ops)
{
  TN* result_tn = Build_TN_Like(opnd1);
  TN* rflags = Rflags_TN();
  ...
  default:
    FmtAssert( FALSE,
	       ("Exp_Fetch_and_XXX: support me now") );
  }
                      ...
                      LABEL_IDX *label, OPS *loop_ops)
{
  ...
  return Exp_Fetch_and_XXX(top, addr, opnd1, mtype, ops,
                           label, loop_ops);
}
                       ...
                       LABEL_IDX *label, OPS *loop_ops)
{
  ...
  default:
    FmtAssert( FALSE,
	       ("Exp_Fetch_and_Nand: support me now") );
  }
...
TN* Exp_Lock_Test_and_Set(TN* addr, TN* opnd1, TYPE_ID mtype, OPS* ops) {
  ...
  if (Is_Target_32bit() && top == TOP_xchgx64)
    FmtAssert(FALSE, ("Exp_Test_Lock_and_Set: 64-bit test-lock-and-set NYI under m32"));
  else
    Build_OP(top, result_tn, opnd1, addr, Gen_Literal_TN(0,4), ops );
...
void Exp_Lock_Release(TN* addr, TYPE_ID mtype, OPS* ops) {
  ...
  if (Is_Target_SSE2())
   ...
   TN *tmp = Build_TN_Of_Mtype(mtype); // get register for temporary value

   // move zero to temporary register
   Build_OP(top, tmp, Gen_Literal_TN(0, 4), ops);
 
   // store value at address addr with offset 0
   Expand_Store(mtype, tmp, addr, Gen_Literal_TN(0, 4), ops);
}
