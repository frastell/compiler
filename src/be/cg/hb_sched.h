...
#ifdef KEY
#define HBS_BALANCE_READY_TYPES		0x04000
#define HBS_BALANCE_UNSCHED_TYPES	0x08000
#define HBS_DROP_UNSCHED_PREFETCHES	0x10000
#define HBS_RETRY			0x20000
#define HBS_NO_REORDER			0x40000
...
typedef struct OPSCH_struct {
  ...
  // Priority adjustment made by the post-schedule analyzer to guide
  // rescheduling.
  INT32 priority;
#endif
} OPSCH;
...
#define OPSCH_priority(opsch)			((opsch)->priority)
#endif
...
#ifdef KEY
// OP is in the ready vector.
#define OPSCH_READY		0x0080
#endif
...
#ifdef KEY
#define OPSCH_ready(opsch)	   (OPSCH_flags(opsch) & OPSCH_READY)
#define Set_OPSCH_ready(opsch)	   (OPSCH_flags(opsch) |= OPSCH_READY)
#define Reset_OPSCH_ready(opsch)   (OPSCH_flags(opsch) &= ~OPSCH_READY)
#endif
...
#ifdef KEY
  int Sched_OP_With_Precious_TN(OP *op);
#endif
...
private:
  ...
  BOOL Can_Schedule_Op (OP *cur_op, INT cur_time, BOOL is_fwd);
#else
  ...
  void Schedule_Block (BB*, BBSCH*, int scheduling_algorithm,
		       BB_OP_MAP priorities = NULL);
  void Retry_Block (BB*, BBSCH*, int scheduling_algorithm);
  BOOL Is_Schedule_Optimal (BB *);
#else
  ...
  BOOL Ignore_Unsched_Succs (OPSCH *opsch);
  BOOL Ignore_Arcs (OPSCH *pred_opsch, OPSCH *succ_opsch, ARC_LIST *arcs);
...
public:
  ...
  void Set_hbs_retry(void) { _hbs_type = (HBS_TYPE) (_hbs_type | HBS_RETRY); }
  void Set_hbs_no_reorder(void) { _hbs_type = (HBS_TYPE) (_hbs_type | HBS_NO_REORDER); }
  void Set_hbs_must_emit_sched(void) { _hbs_type = (HBS_TYPE) (_hbs_type | HBS_MUST_EMIT_SCHED); }
#endif
  ...
  BOOL HBS_Retry(void) const { return _hbs_type & HBS_RETRY; }
  BOOL HBS_No_Reorder(void) const { return _hbs_type & HBS_NO_REORDER; }
  BOOL HBS_Relax_Reganti(void) const { return _hbs_type & HBS_RELAX_REGANTI; }
  BOOL HBS_Must_Emit_Sched(void) const { return _hbs_type & HBS_MUST_EMIT_SCHED; }
#endif
  ...
  BOOL Sched_Prefetch_First(void)	    { return _sched_prefetch_first; }
#endif
...
#ifdef KEY
  BOOL Schedule_BB (BB*, BBSCH*, int scheduling_algorithm = -1);
#else
