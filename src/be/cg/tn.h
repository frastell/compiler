...
#endif
#ifdef TARG_MIPS
#define TN_VECTOR	0x4000  /* TN holds paired single, not double */
#endif
...
#ifdef TARG_MIPS
#define       TN_is_vector(r)  (TN_flags(r) &   TN_VECTOR)
#define   Set_TN_is_vector(r)  (TN_flags(r) |=  TN_VECTOR)
#define Reset_TN_is_vector(r)  (TN_flags(r) &= ~TN_VECTOR)
#endif
...
inline TN *Build_TN_Like(TN *tn)
{
  ...
  // Propagate fpu-int and is_vector flags
#ifdef TARG_MIPS
  TN_flags(new_tn) |= (TN_flags(tn) & (TN_FPU_INT | TN_VECTOR));
#else
...
inline TN *Build_TN_Of_Mtype(TYPE_ID mtype)
{
  ...
  TN *tn = Gen_Register_TN (rc, MTYPE_RegisterSize(mtype) );
#ifdef TARG_MIPS
...
extern	void  fPrint_TN ( FILE *f, const char *fmt, const TN *tn);
#pragma mips_frequency_hint NEVER fPrint_TN
/* Print TN to the trace file TFile */
extern	void   Print_TN (const TN *tn, BOOL verbose, FILE *f = TFile);
...
/* Print all TNs */
extern	void   Print_TNs (FILE *f = TFile);
