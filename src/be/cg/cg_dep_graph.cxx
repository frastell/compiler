...
// Insert dependence info for a new noop with no attached arcs (for bug 13083)
void CG_DEP_New_Noop(OP *op) 
{
  BB_OP_MAP dep_map = (BB_OP_MAP) BB_MAP_Get(_cg_dep_op_info, OP_bb(op));
  BB_OP_MAP_Set(dep_map, op, new_op_info());
}
 ...
 */
static BOOL 
symbolic_addr_subtract (OP *pred_op, OP *succ_op, SAME_ADDR_RESULT *res)
{
  ST *pred_initial_sym;
  ST *succ_initial_sym;
  ST *pred_sym;
  ...
  OP *pred_root = addr_base_offset (pred_op, &pred_initial_sym, 
                                    &pred_sym, &pred_base, 
                                    &pred_offset);
  OP *succ_root = addr_base_offset (succ_op, &succ_initial_sym, 
                                    &succ_sym, &succ_base, 
                                    &succ_offset);

  if (pred_root && pred_base && succ_root && succ_base) {
    if (   pred_sym && succ_sym 
        && (ST_sclass(pred_initial_sym) != SCLASS_UNKNOWN) 
        && (ST_sclass(succ_initial_sym) != SCLASS_UNKNOWN)
       )
    {
      if (   (pred_sym != succ_sym) 
	...
	*diff = TN_offset (pred_tn) - TN_offset (succ_tn);
	...
	if (   (!parc && !sarc) 
	  ...
	  *diff = 0;
      ...
      if (!parc && !sarc) {
        *diff = 0;
      ...
      else if (parc && sarc && ARC_pred (parc) == ARC_pred (sarc)) {
        INT omega = addr_omega (succ_op, si) - addr_omega (pred_op, pi);
        if (OP_Precedes (succ_op, pred_op)) {
          omega += 1;
        }
        ...
        if (   OP_iadd (op)
            && OP_result (op, 0) == pred_tn
            && OP_opnd (op, 0) == pred_tn
            && TN_is_constant (OP_opnd (op, 1))
            && TN_has_value (OP_opnd (op, 1))
           )
        {
          *diff = omega * TN_value (OP_opnd (op, 1));
	}
        else {
          *diff = 0;
        }
 ...
 */
{
  ...
  OP_Base_Offset_TNs (pred_op, &pred_base, &pred_offset);
  OP_Base_Offset_TNs (succ_op, &succ_base, &succ_offset);
 ...
 */
{
  ...
  if (pred_op == NULL && succ_op == NULL) {
    ...
    if (repeat_count > 2)
      DevWarn("verify_mem: last warning reoccurred %" SCNd64 " times "
  ...
  if (repeat_count > 0 &&
    ...
    if (repeat_count > 2)
      DevWarn("verify_mem: last warning reoccurred %" SCNd64 " times", repeat_count);
    last_result = result;
...
#ifdef KEY
  BOOL is_load_store = ((OP_load(pred_op) && OP_store(succ_op)) ||
			(OP_load(succ_op) && OP_store(pred_op)));
#ifdef TARG_X8664
  is_load_store |= ((OP_load_exe(pred_op) && OP_store(succ_op)) ||
		    (OP_load_exe(succ_op) && OP_store(pred_op)));
#endif
  if (is_load_store &&				// SiCortex 5453
      CG_DEP_Ignore_Loop_Loadstore_Dep &&
      BB_innermost(OP_bb(pred_op))) {
    return FALSE;
  }
#endif	// KEY
...
/* Handle same op in the non cyclic case. */
  ...
  if (   memread 
      && (   !include_memread_arcs 
          || CGSPILL_Is_Spill_Op (pred_op) 
          || CGSPILL_Is_Spill_Op(succ_op)
         )
     )
  {
    return_value =  FALSE; goto return_point;
...
add_home_mem_arcs_for_op (OP *op, TN *home_tn, int prev_mem_ops_seen)
{
  ...
  for (i = 0; i < num_mem_ops; i++) {
    ...
    if (wn != NULL &&
	ALIAS_RESULT_positive(Aliased(Alias_Manager, TN_home(home_tn), wn))) {
	...
	for (i = tn_defs.size()-1; i >= 0; i--) {
	  ...
	  if (!OP_has_disjoint_predicate_cyclic(op_vec[def_num], op))
	    new_arc(CG_DEP_REGIN, op_vec[def_num], op, omega, opnd, FALSE);
          ...
          /* Prune arcs for cond_def or same_res OPs.
           * i.e.
           * TN100 <- op1
           * TN100 <- op2 TN100 cond_def
           *
           *       <- op3 TN100
           *
           * The REGIN arc from op1 to op3 is not necessary, because
           * the REGIN arc from op1 to op2 and the REGIN arc from op2
           * to op3 are enough to guarantee the right order during
           * scheduling.
           *
           * This is only true for MIPS, because the machine description
           * for x86 conditional defined OPs is different.
           */
          if (   OP_cond_def (op_vec[def_num]) 
              || OP_same_res (op_vec[def_num]))
          {
            break;
          }
          #endif
...
//
CYCLIC_DEP_GRAPH::CYCLIC_DEP_GRAPH (BB *body) :
  _body (body) 
{
...
#ifdef TARG_MIPS
  // 15163: CG_DEP_Address_Analyze doesn't work on cyclic graphs for MIPS.
  // Disable CG_DEP_Addr_Analysis until fixed (just like CIO already does).
  BOOL save_CG_DEP_Addr_Analysis = CG_DEP_Addr_Analysis;
  if (cyclic) CG_DEP_Addr_Analysis = FALSE;
#endif
