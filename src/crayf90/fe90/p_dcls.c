   ...
   if (LA_CH_VALUE == COMMA) {
	...
	for (int i = 0; i < 2; i += 1) {
	  if (matched_specific_token(Tok_Kwd_Private, Tok_Class_Keyword)) {
	     access = Private;
	     access_set = TRUE;
	     access_set_line = TOKEN_LINE(token);
	     access_set_col = TOKEN_LINE(token);
	     is_private += 1;
	  }
	  else if (matched_specific_token(Tok_Kwd_Public, Tok_Class_Keyword)) {
	     access = Public;
	     access_set = TRUE;
	     access_set_line = TOKEN_LINE(token);
	     access_set_col = TOKEN_LINE(token);
	...
	if (is_private > 1 || is_public > 1 || is_bind > 1) {
	   ...
	   PRINTMSG(access_set_line, 596, Error, access_set_col, str);
	   access_set = FALSE;
	}
...

{
   ...
   while (LA_CH_VALUE == SLASH) {
	 ...
	 if (AT_OBJ_CLASS(grp_attr) != Namelist_Grp) {
	     AT_OBJ_CLASS(grp_attr)	= Namelist_Grp;
...
# endif
	 ...
	 /* Note that bind(c) must not be done here, but above */

         if (attr_list & (1 << Protected_Attr)) {
            merge_protected(TRUE, id_line, id_column, attr_idx);
         }
   ...
   do {
	 ...
	 case Tok_Kwd_Bind:
	    ...
	    break;
         ...
         case Tok_Kwd_Protected:

            if (CURR_BLK != Module_Blk) {
               issue_attr_blk_err("PROTECTED");
            }
            else {
               err_in_list	= err_attrs[Protected_Attr] & attr_list;
               attr_list	= attr_list | (1 << Protected_Attr);

               if (err_in_list) {
                  issue_attr_err(Protected_Attr, err_in_list);
                  err_list	= err_list | (1 << Protected_Attr);
               }
               else {
                  AT_PROTECTED(AT_WORK_IDX)	= TRUE;
               }
            }
            break;
#endif /* KEY Bug 14150, 5091 */
