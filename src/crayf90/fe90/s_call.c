...
#ifdef KEY /* Bug 11250 */
/* For use in situations where we create Loc_Opr(array) and the actual code is
 * something like Loc_Opr(Subscript_Opr(array, Dv_Access_Low_Bound(array)).
 * We don't need a bounds check on Subscript_Opr since we're just taking the
 * address, not accessing the array; and since cray/fe90 doesn't generate a
 * bounds check, cwh_addr_insert_bounds_check() in sgi/cwh_addr.c will try
 * to emit one, and will erroneously complain if the array is zero-size.
 *
 * opnd		IL for Subscript_Opr
 */
static void
dont_bounds_check_base_subtree(opnd_type *base_opnd) {
  if (OPND_FLD(*base_opnd) == IR_Tbl_Idx) {
    int base_idx = OPND_IDX(*base_opnd);
    if (IR_OPR(base_idx) == Subscript_Opr) {
      IR_BOUNDS_DONE(base_idx) = TRUE;
    }
  }
}
#endif /* KEY Bug 11250 */
...
CONTINUE:
         ...
         if (is_function) {
            ...
            if (! no_func_expansion) {
               ...
               if (AT_IS_INTRIN(spec_idx) &&
                  ...
                  copy_subtree(result_opnd, &save_result_opnd, 0, 0);
         ...
         if (! no_func_expansion) {
            ...
            if (! is_function) {
               ...
               ok = final_arg_work(&opnd, spec_idx, num_args, NULL,
#ifdef KEY /* Bug 8090 (Sicortex) */
	         ir_idx
#endif /* Bug 8090 (Sicortex) */
		 ) && ok;
...
|*      num_args 	- num args.                                           *|
|*      elemental_exp_desc -                                                  *|
|*      call_ir_idx	- IR_Tbl_Idx for call
|*									      *|
|* Output parameters:							      *|
|*	NONE								      *|
|*									      *|
...
#ifdef KEY /* Bug 5091 */
		/* Exclude "protected" */
                arg_info_list[info_idx].ed.constant ||
#endif /* KEY Bug 5091 */
...
# endif
            ...
            make_base_subtree(&opnd, &base_opnd, &unused1, &unused2);
#ifdef KEY /* Bug 11250 */
            dont_bounds_check_base_subtree(&base_opnd);
#endif /* KEY Bug 11250 */
...
#endif /* KEY Bug 8117 */
# if (defined(_TARGET_OS_IRIX) || defined(_TARGET_OS_LINUX) || defined(_TARGET_OS_DARWIN))
#ifdef KEY /* Sicortex bug 7156 */
            if (clear_pt_unique_mem(tmp_idx)) {
#endif /* KEY Sicortex bug 7156 */
	      ATD_NOT_PT_UNIQUE_MEM(tmp_idx) = TRUE;
#ifdef KEY /* Sicortex bug 7156 */
            }
#endif /* KEY Sicortex bug 7156 */
...
#ifdef KEY /* Sicortex bug 7156 */
	       /* If this is a contiguous section of an allocatable object,
	        * then the resulting elements are contiguous--no need to check
		* the flag in the dope vector at runtime */
               if (exp_desc->allocatable_base) {
...
# endif
	 ...
	 /* Bug 12711 (Sicortex 4400): Also deallocate temp if it's a 
	  * structure containing allocatable components. Could combine
	  * this test with the one above and call dealloc_allocatables()
	  * for both, but until we're sure this fix is correct, keep them
	  * separate. */
	   ...
	   SH_P2_SKIP_ME(curr_stmt_sh_idx)   = TRUE;
	   boolean first = TRUE;
	   dealloc_allocatables(line, col, tmp_idx, AT_Tbl_Idx, tmp_idx,
	     FALSE, &first);
	   curr_stmt_sh_idx = save_curr_stmt_sh_idx;
	 }
...
static void check_call_for_global_def(int	list_idx,
				      ...
				      int	num_dargs,
#ifdef KEY /* Bug 8090 (Sicortex) */
   /* Messages about conflicting global definitions and references must use
    * the line/col number from the call IR, not the line/col number from the
    * attribute, for each reference, because the attribute might have been
    * created for a correct reference which is then followed by an
    * erroneous reference. */
				      int	call_ir_idx
#endif /* Bug 8090 (Sicortex) */
				      )
...
#ifdef KEY /* Bug 8090 (Sicortex) */
   /* We want the line/col of the call IR for this particular call, not the
    * line/col of the attribute of the called procedure */
   GA_DEF_LINE(*ga_idx) = IR_LINE_NUM(call_ir_idx);
   GA_DEF_COLUMN(*ga_idx) = IR_COL_NUM(call_ir_idx);
#endif /* Bug 8090 (Sicortex) */
