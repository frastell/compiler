...
cwh_stmt_call_helper(INT32 num_args, TY_IDX ty, INT32 inline_state, INT64 flags)
{
  ...
  if ((ST_auxst_is_elemental(st)) && (TY_mtype(ts) != MTYPE_V)) {
     ...
     /* For struct by value, wn is null but rbtype1 gives type; for scalar
      * result of elemental, Get_Return_Mtypes() may have set
      * return_via_first_arg=1 and not set mtype (or, consequently, rbtype1)
      *earlier: use WN_rtype() instead */
     opc = cwh_make_typed_opcode(OPR_COMMA, (wn ? WN_rtype(wn) : rbtype1),
       MTYPE_V);
...
#else /* KEY Bug 8191, 9520, 10282 */
     wn  = cwh_stmt_return_scalar(rt,NULL,ts,FALSE);
     opc = cwh_make_typed_opcode(OPR_COMMA,rbtype1,MTYPE_V);
     wn  = WN_CreateComma(opc,block,wn);
     cwh_stk_push_typed(wn,WN_item,ty);
#endif /* KEY Bug 8191, 9520, 10282 */
