...
SUMMARIZE<program>::Summarize (WN *w)
{
  ...
  if (!Has_alt_entry ()) {
    ...
    //
    // KEY bug 14679 required introduction of ipa_util namespace.
    Chi_To_Idx_Map =
	CXX_NEW (CHI_CR_TO_INT_MAP (113, ipa_util::ptr_hash<CODEREP> (),
				    std::equal_to<CODEREP*> (), &Temp_pool),
		 &Temp_pool);
    Phi_To_Idx_Map =
	CXX_NEW (PHI_NODE_TO_INT_MAP (113, ipa_util::ptr_hash<PHI_NODE> (),
...
Count_WN (WN * wn, INT32& bbs, INT32& stmts, INT32& calls)
{
    ...
    } else if ((rtype == MTYPE_FQ || rtype == MTYPE_F16 ||
		rtype == MTYPE_CQ || rtype == MTYPE_C16) &&
...
// Return array-type if any through TYPE.
static inline BOOL
...
#ifdef KEY
    // sct 7223: analyze a dope vector to find an array
    if (TY_kind(array_ty) == KIND_STRUCT &&
        strncmp(TY_name(array_ty), ".dope.", 6) == 0) {
      // first field of the dope vector
      array_ty = FLD_type(TY_fld(array_ty));
    }
#endif
...
#ifdef KEY
          if (array_ty != TY_IDX_ZERO)
            actual->Set_ty(array_ty);
          else
#endif
