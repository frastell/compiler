...
#if defined(KEY) && !defined(_STANDALONE_INLINER) && !defined(_LIGHTWEIGHT_INLINER)
    // bugs 3060, 14359
    // If debugging is ON and source PU order is maintained, then update
    // filenum to be that of the callee.
    // Else give everything the linenum of the callsite
    // bug 6170: for lw-inliner, maintain the callee's line# information
    if (OPERATOR_has_next_prev(WN_operator(wn))) {
      SRCPOS lin = WN_Get_Linenum (wn);
      if (IPA_Enable_Source_PU_Order &&
          IPA_Enable_Debug &&
          SRCPOS_filenum (lin)) {
        Is_True (aux.header_file_offset != HEADER_FILE_INVALID_OFST,
                 ("Invalid header file offset under IPA debugging"));
        // If there is a header file offset, update the filenum and
        // write back the source position.
        if (aux.header_file_offset) {
          SRCPOS_filenum (lin) = SRCPOS_filenum (lin) + aux.header_file_offset;
          WN_Set_Linenum (wn, lin);
        }
      }
      else
...
//
	    ...
	    // (1)
	    INITV_IDX first = INITO_val (PU_misc_info (Get_Current_PU ()));
...
#ifdef KEY // bug 6674, SiCortex 5074
...
IPO_INLINE::Process()
{
  INT32 caller_file_index = Caller_node ()->File_Index ();
  INT32 callee_file_index = Callee_node ()->File_Index ();
  ...
  BOOL same_file =    // caller & callee from same file?, used many places
      (caller_file_index == callee_file_index) ;


  IPO_INLINE_AUX aux_info (Callee_node ()->Num_Formals (), &Ipo_mem_pool);

#if defined(KEY) && !defined(_STANDALONE_INLINER) && !defined(_LIGHTWEIGHT_INLINER)
  if (IPA_Enable_Source_PU_Order && IPA_Enable_Debug && !same_file)
  {
    if (Header_File_Offsets[caller_file_index][callee_file_index] ==
        HEADER_FILE_INVALID_OFST)
    {
      Header_File_Offsets[caller_file_index][callee_file_index] =
          DST_copy_directories_and_files
             (IP_FILE_HDR_dst(Callee_node()->File_Header()),
              IP_FILE_HDR_dst(Caller_node()->File_Header()),
              MEM_phase_nz_pool_ptr);
    }

    aux_info.header_file_offset =
         Header_File_Offsets[caller_file_index][callee_file_index];
  }
#endif
