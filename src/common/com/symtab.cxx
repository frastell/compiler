...
#ifdef KEY
// if align is not a power of 2, return the log (base 2) of the next
// higher power of 2
UINT
Get_log_base2 (UINT align)
{
  BOOL power_of_two = TRUE;
  UINT val = 0;

  Is_True (align, ("Invalid alignment"));
  ...
  while (align > 1) {
    align >>= 1;
    if (power_of_two && (align % 2) != 0)
      power_of_two = FALSE;
    val++;
  }

  if (!power_of_two)
    val++; // next higher power
...
ST::Print (FILE *f, BOOL verbose) const
{
	...
	if (base_idx != 0) {
	    ...
	    fprintf (f, "\t\tAddress: %" SCNd64 "(%s<%d,%d>)  ", offset,
...
{
    fprintf (f, "\t%6" SCNd64 "  %-8s\t", ofst, &Str_Table[name_idx]);
...
{
    fprintf (f, "size %" SCNd64 ", align %d, flags 0x%04x, section %d, scninfo %d\n",
...
Print_global_symtab (FILE *f)
{
    ...
    fprintf (f, "%sString table size = %" SCNd64 "\n", DBar, STR_Table_Size());
    fprintf (f, "%s\n", DBar);
} // Print_global_symtab
...
New_Scope (SYMTAB_IDX level, MEM_POOL *pool, BOOL reserve_index_zero)
{
	...
	st_tab->New_entry ((UINT&) dummy_idx);
